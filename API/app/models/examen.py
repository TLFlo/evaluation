...
class Examen(Base):
    ...
    date_examen: Mapped[date] = mapped_column(
        Date,
        nullable=False
    )
    ...
    matiere: Mapped[str] = mapped_column(
        String(50),
        nullable=False
    )

    professeur: Mapped[str] = mapped_column(
        String(100),
        nullable=False
    )
    ...
    salles: Mapped[list["SalleExamen"]] = relationship(
        back_populates="examen"
    )

    classes_assoc: Mapped[list["ExamenClasse"]] = relationship(
        back_populates="examen"
    )