...
from app.schemas.affectation import AffecterGroupeRequest
...
def affecter_groupe(
    ...
    classe_examen = db.query(ExamenClasse).filter(
    ...
    if classe_examen is None:
        raise HTTPException(
            status_code=400,
            detail="Cette classe n'est pas concernée par cet examen"
        )

    # 4. Vérifier que la salle existe
    salle = db.query(SalleExamen).filter(
        SalleExamen.id == data.salle_id
    ).first()

    if salle is None:
        raise HTTPException(
            status_code=404,
            detail="Salle d'examen introuvable"
        )
    ...
    if salle.id_examen != examen_id:
        raise HTTPException(
            status_code=400,
            detail="Cette salle n'appartient pas à cet examen"
        )
    ...
    if not etudiants:
        raise HTTPException(
            status_code=404,
            detail="Aucun étudiant trouvé dans ce groupe"
        )

    # 7. Vérifier les étudiants déjà affectés
    matricules = [e.matr for e in etudiants]

    participations_existantes = db.query(
        ParticipationExamen
    ).filter(
    ...
    if participations_existantes:
        matricules_existants = [
            p.etudiant_matr
            for p in participations_existantes
        ]
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Erreur lors de l'affectation du groupe"
        )
